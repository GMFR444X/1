 ...
 if (process.argv.length < 7){console.log(` node gfc.js target time rate thread proxy.txt`); process.exit();}
 const headers = {};
 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(`system: ${getCurrentTime()} Title: ${getTitleFromHTML(data)} ({status})`);
    })
    .catch((error) => {
      if (error.message === 'Request timed out') {
        console.log(`system: ${getCurrentTime()} Request Timed Out`);
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(`system: ${getCurrentTime()} Title: ${extractedTitle} {error.response.status}`);
      } else {
        console.log(`system: ${getCurrentTime()} ${error.message}`);
      }
    });
}
